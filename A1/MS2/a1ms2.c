...
int main(void)
{
    ...
    person.birthYear = getIntFromRange(currentYear() - 110, currentYear() - 18);
    printf("Enter the household Income: $");
    person.houseHoldIncome = getPositiveDouble();
    printf("Enter the country (30 chars max.): ");
    getCString(person.cntyName, 1, 30);
    printf("\n");
    // ---------------------------------------------------------
...
void displayAccount(const struct Account account, const struct UserLogin login, const struct Person person)
{
    ...
    printf("%05d %-9s %-15s %5d %11.2lf %-10s %-10s %8s\n",
           ...
           person.fullName, person.birthYear,
           person.houseHoldIncome, person.cntyName,
