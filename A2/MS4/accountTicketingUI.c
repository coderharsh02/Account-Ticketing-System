...
// saves data to accounts.txt file from accounts array
...
// // saves data to tickets.txt file from tickets array
...
void menuAgent(struct AccountTicketingData *accTicPtr, const struct Account *accPtr)
{
    ...
    do
    {
        ...
        case 8:
            ...
            for (i = 0; i < accTicPtr->TICKET_MAX_SIZE; i++)
            {
                if (accTicPtr->tickets[i].ticNum == 0 && !found)
                {
                    ticIndex = i;
                    found = 1;
                }
                ...
                if (index > -1)
                {
                    if (accTicPtr->accounts[index].accountType == 'C')
                    {
                        ...
                        if (c == 'Y')
                        {
                            ...
                            accTicPtr->tickets[ticIndex].ticMesCounter = 0;
        ...
        case 0:
            ...
            printf("   %d account(s) saved.\n", saveAccounts(accTicPtr->accounts, accTicPtr->ACCOUNT_MAX_SIZE));
            printf("   %d ticket(s) saved.\n", saveTickets(accTicPtr->tickets, accTicPtr->TICKET_MAX_SIZE));
            printf("### LOGGED OUT ###\n\n");
            break;
        }
    } while (userChoice);
